...
and one parameter is specified by indices. Negative values are used to specify reversed indices. Thus, we here obtain the two first and the two latest years in the data.
...
This example shows the use of aggregation in age groups and aggregated timeseries for the new Norwegian municipality structure from 2020. Note the url where /en is replaced by /no. That returns labels in Norwegian instead of in English.
...
1) The name of the filter and the IDs are not shown in PxWebApi metadata, only in the code "API Query for this table".
...
To convert an original JSON API query it is possible to use the [PxWebApiData call creator](https://radbrt-px-converter-px-converter-3mdstf.streamlit.app).

## Readymade datasets
Statistics Norway also provide an API with readymade datasets, available by http GET.
Use the parameter `getDataByGET = TRUE`. By changing to `lang=no` in the URL you get the label version in Norwegian.
...
## Eurostat data
Eurostat REST API offers JSON-stat version 2. Even if it not PxWeb, it is possible to use this package to obtain data from Eurostat by using `getDataByGET`. 
...
urlEurostat <- ApiData12("https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/prc_hicp_mv12r?format=JSON&lang=EN&lastTimePeriod=2&coicop=CP00&geo=NO&geo=EU", getDataByGET = TRUE)
urlEurostat
comment(urlEurostat)
```
